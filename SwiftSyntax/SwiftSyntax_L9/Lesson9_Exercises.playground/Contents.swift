...
var reversedSurnames = surnames.sort( { (name1: String, name2: String)-> Bool in
    return name1 > name2
})
...
let conciseAverages = battingAverages.sort( {
    return $1 > $0
})
...
numbersAsStrings.filter( {  (numberString:String) -> Bool in
    Int(numberString)!%12 == 0
})

//: __3b.__
//: Rewrite the filtering closure expression to be as concise as possible.
numbersAsStrings.filter( {
    Int($0)!%12 == 0
})
//: __Problem 4__
//:
