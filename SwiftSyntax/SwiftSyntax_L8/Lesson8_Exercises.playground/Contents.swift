...
class Teenager : Babysitter{
    ...
    func playCandyland(var numberOfTimes: Int) {
        
        while numberOfTimes > 0 {
            print("play in the candy land")
            numberOfTimes--
        }
    }
...
var teen1 = Teenager(age: 10, responsible: true, patience: .High)
teen1.playCandyland(5)
teen1.read("The Jungle Book", firstLine: "first line", asleep: false)
...
var cuteCat = Animal(species: "Cat", numberOfLegs: 4)
cuteCat.frolick()
//: __Problem 3__
//:
...
class ScrubJay : Hoarder {
    ...
    func cache(foodItem:String)->String {
        return "save the \(foodItem) in the wood"
    }
    
    func pilfer() -> String {
        return "Sorry.. I have to pilfer your fooditem"
    }
}
...
var firstRoommate = Roommate(hungry: true,name: "mike")
firstRoommate.chop("carrot")
firstRoommate.rinse("carrot")
...
extension Minion : DirtyDeeds {
    func cheat() {
        print("\(self.name) is cheating on the test")
    }
    
    func steal() {
        print("\(self.name) is stealing that lady's purse")
    }
}
