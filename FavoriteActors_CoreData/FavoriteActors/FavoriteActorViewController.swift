...
class FavoriteActorViewController : UITableViewController, ActorPickerViewControllerDelegate {
    ...
    func actorPicker(actorPicker: ActorPickerViewController, didPickActor actor: Person?) {
        ...
        if let newActor = actor {
            ...
            // Whenever a context is created it is associated with a particular thread. 
            // The context that we are using was created in the AppDelegate, and it has been created in such as 
            // way that it should be used exclusively on the main thread.
            dispatch_async(dispatch_get_main_queue()) {
                let newActor = Person(dictionary: dictionary, context: self.sharedContext)
